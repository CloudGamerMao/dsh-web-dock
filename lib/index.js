// Host half of the dsh-web-dock plugin (Phase 5 POC).
//
...
export const name = 'dsh-web-dock'
...
// v6.25: persisted to disk (~/.dsh/dsh-web-dock/asset-cache) so the FIRST
...
const ASSET_CACHE_DIR = path.join(os.homedir(), '.dsh', 'dsh-web-dock', 'asset-cache')
...
function loadAssetCache() {
  try {
    ...
    ctxLogger?.info?.('[dsh-web-dock] loaded ' + n + ' cached assets from disk')
  } catch (e) {}
}
...
const JAR_FILE = path.join(os.homedir(), '.dsh', 'dsh-web-dock', 'session.json')
let lastJarSave = 0
...
function loadJar() {
  try {
    ...
    ctxLogger?.info?.('[dsh-web-dock] loaded cookie jar: ' + cookieJar.size + ' session(s) from ' + JAR_FILE)
  } catch {
...
function saveJarNow() {
  try {
    ...
    ctxLogger?.warn?.('[dsh-web-dock] cookie jar save failed: ' + (e && e.message ? e.message : String(e)))
  }
}
...
export function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    ctx.logger?.info?.('[dsh-web-dock] registered /__dsweb-test prefix route (+proxy+sw+status)')
    return () => dispose()
  }, 'dsh-web-dock: proxy/sw/status routes')
}
